    ...
    for (int i = 0; i<m1; i++){
        ...
        for (int i = 0; i<(int)v.size(); i++){
            ...
            for (int j = i+1; j<(int)v.size(); j++){
                ...
                for (auto x: m[v[i]]){
                    if (x == v[j]){
                        // For whatever j, makes the update invalid, we keep swaping it with i untill it becomes valid.
                        swap(v[i], v[j]);
                        flag = 0;
                        t = 0;
                        break; // We swap then break
                    }
                }
                if (!t) i--; // If break statement was reached we process index i again, as it has changed.
            }
        }
