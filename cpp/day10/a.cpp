    ...
    for (int i = 0; i<n; i++){
        for (int j = 0; j<n; j++){
            if (a[i][j] == 0){
                ...
                q.push({i, j});
                set<pair<int, int>> s; // To store the distinct 9s we can reach from trailhead at (i, j).
                while (!q.empty()){
                    ...
                    for (auto x: diff){
                        ...
                        if (xn >= 0 && xn <=n-1 && yn>=0 && yn<=n-1){
                            // If the new coordinates point to 9 and previous coordinate was 8, we insert the coordinates of 9 into set
                            if (a[xn][yn] == 9 && val == 8){
                                s.insert({xn, yn});
                            }
                            // If not equal 9
                ...
                // Adding the number of distinct 9s reachable to ans
                ans += (int)s.size();
            }
        }
    }
